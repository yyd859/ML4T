...
class RTLearner(object):
    ...
    def determine_best_split(self,array):
        ...
        best_split_column = np.random.choice(n_features)
        best_split_value = np.random.uniform(min(x[:, best_split_column]), max(x[:, best_split_column]))
    ...
    def build_decision_tree(self,array):
        ...
        max_value = np.max(x[:, best_split_column])
        min_value = np.min(x[:, best_split_column])

        if best_split_column is None:
            mean_target = np.mean(array[:, -1])
            return np.array([mean_target])
        if best_split_value == min_value:
            mean_target = np.mean(array[:, -1])
            return np.array([mean_target])
        if best_split_value == max_value:
            mean_target = np.mean(array[:, -1])
            return np.array([mean_target])
